...
module.exports = {
  ...
  enumerateDevices() {
    if (!addon) return [];
    return addon.enumerateDevices();
  },

  async openDevice(options) {
    if (!addon) throw new Error('DeckLink addon not loaded');
    return addon.openDevice(options);
  },

  async displayFrame(handle, videoBuf) {
    if (!addon) throw new Error('DeckLink addon not loaded');
    return addon.displayFrame(handle, videoBuf);
  },

  async writeAudio(handle, audioBuf) {
    if (!addon) throw new Error('DeckLink addon not loaded');
    return addon.writeAudio(handle, audioBuf);
  },

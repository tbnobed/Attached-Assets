...
receiver.on('api-request', async ({ clientId, reqId, path, body }) => {
    ...
    switch (path) {
      case '/api/devices': {
        ...
        for (const [key, val] of Object.entries(DISPLAY_MODES)) {
          displayModes[key] = val.label;
        }
        const outputStatus = {};
        for (const [idx, out] of deckLinkManager.outputs) {
          outputStatus[idx] = { active: !!out.active, mode: out.modeKey || null };
        }
        response = { success: true, devices, displayModes, outputStatus };
        break;
      }
      case '/api/output/start': {
        const { deviceIndex, mode } = body;
        if (deviceIndex === undefined) {
          response = { success: false, error: 'deviceIndex required' };
        } else {
          response = await deckLinkManager.startOutput(deviceIndex, mode || '1080p2997');
        }
        break;
      }
      ...
      case '/api/assign': {
        ...
        if (userId === undefined || deviceIndex === undefined) {
          response = { success: false, error: 'userId and deviceIndex required' };
        } else {
          response = await deckLinkManager.assignParticipant(userId, deviceIndex);
        }
        break;
      }
      case '/api/unassign': {
        const { userId } = body;
        if (userId === undefined) {
          response = { success: false, error: 'userId required' };
        } else {
          deckLinkManager.unassignParticipant(userId);
          response = { success: true };
        }
      ...
      case '/api/status': {
        response = {
          ...
          ndi: ndiManager.getStatus ? ndiManager.getStatus() : null,
        };
        break;
      }
      default:
        response = { success: false, error: `Unknown API path: ${path}` };
    }
  } catch (err) {
    response = { success: false, error: err.message };
  }
  receiver.sendApiResponse(clientId, reqId, response);
});

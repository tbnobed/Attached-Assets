...
class RecorderManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.videoFrameRate = options.videoFrameRate || 30;
    this.audioSampleRate = options.audioSampleRate || 48000;
    this.audioChannels = options.audioChannels || 1;
    this.videoCodec = options.videoCodec || 'libx264';
    this.videoPreset = options.videoPreset || 'fast';
  ...
  startRecording(userId, displayName) {
    ...
    console.log(`[Recorder] Recording queued for ${displayName}: ${filename}`);
    return recorder;
  }
  ...
  _spawnFfmpeg(recorder, width, height) {
    const ffmpegArgs = [
      ...
      '-i', 'pipe:0',
      '-f', 's16le',
      '-ar', String(this.audioSampleRate),
      '-ac', String(this.audioChannels),
      '-i', 'pipe:3',
      '-c:v', this.videoCodec,
    ...
    try {
      ...
      ffmpeg.stderr.on('data', (data) => {
        ...
        if (msg.includes('Error') || msg.includes('error')) {
          console.error(`[Recorder] FFmpeg error for ${recorder.displayName}:`, msg.trim());
        }
      });
      ...
      ffmpeg.on('error', (err) => {
        console.error(`[Recorder] FFmpeg process error for ${recorder.displayName}:`, err.message);
        recorder.active = false;
        this.recorders.delete(recorder.userId);
      ...
      console.log(`[Recorder] FFmpeg started for ${recorder.displayName}: ${width}x${height}`);
    } catch (err) {
      console.error(`[Recorder] Failed to spawn FFmpeg for ${recorder.displayName}:`, err.message);
      recorder.active = false;
      this.recorders.delete(recorder.userId);
      this.emit('recording-error', { userId: recorder.userId, displayName: recorder.displayName, error: err.message });
    }
  ...
  stopRecording(userId) {
    ...
    try {
      ...
      console.error(`[Recorder] Error stopping FFmpeg for ${recorder.displayName}:`, err.message);
      try { recorder.ffmpeg.kill('SIGTERM'); } catch (e) {}
    }
  ...
  writeVideoFrame(userId, frameBuffer, width, height) {
    ...
    try {
      ...
      console.error(`[Recorder] Error writing video for ${recorder.displayName}:`, err.message);
    }
    return false;
  }
  ...
  writeAudioData(userId, audioBuffer) {
    ...
    try {
      ...
      console.error(`[Recorder] Error writing audio for ${recorder.displayName}:`, err.message);
    }
    return false;
  }
  ...
  getRecordingStatus(userId) {
    ...
    try {
      if (fs.existsSync(recorder.filepath)) {
        fileSize = fs.statSync(recorder.filepath).size;
      }
    } catch (e) {}
